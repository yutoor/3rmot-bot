...
// دالة دخول الروم مع فتح السماعة (selfDeaf: false)
...
client.on("voiceStateUpdate", async (oldState, newState) => {
    // رصد دخول عضو جديد (ليس بوت) للروم
    if (newState.channelId === SUPPORT_VC_ID && !newState.member.user.bot && oldState.channelId !== newState.channelId) {
        ...
        if (connection) {
            ...
            // تحديد المسار بدقة لبيئة Linux في الاستضافة
            ...
            player.on('error', error => console.error(`❌ خطأ في التشغيل: ${error.message}`));
            player.on(AudioPlayerStatus.Playing, () => console.log('🎵 المقطع يشتغل الآن!'));
        }
    }
});
...
client.on("messageCreate", async (message) => {
    ...
    const mainEmbed = new EmbedBuilder()
        ...
        .setDescription("# نظام 3RMOT الصوتي\nتم تحديث المسارات وفك تشفير الصوت.")
        .setImage(moonImage);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isButton() && interaction.customId === 'btn_reconnect_vc') {
        ...
        await interaction.reply({ content: "✅ تم تحديث حالة الاتصال الصوتي!", ephemeral: true });
    }
});
